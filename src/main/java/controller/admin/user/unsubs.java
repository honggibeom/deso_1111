...
@WebServlet("/admin/user/unsubs")
public class unsubs extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(admin==null){
			...
			//회원리스트
			String table = "탈퇴회원";
			List<Member> list = am.list(kind,keyword,table,startRow,endRow);
			
			// 전체 게시글 가져오기 위한 ( 전체 글 개수 세는 것)
			int listCount = am.listCount(kind,keyword,table,startRow,endRow);
			...
			RequestDispatcher dis = request.getRequestDispatcher("./unsubs.jsp");
